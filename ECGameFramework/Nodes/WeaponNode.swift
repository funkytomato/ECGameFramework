...
Abstract:
An `SKNode` subclass used as a container for the visual components that make up a `TaskBot`'s beam.
*/
...
class WeaponNode: SKNode, ResourceLoadableType
{
    ...
    func update(withWeaponState state: GKState, source: TaskBot, target: TaskBot? = nil)
    {
        ...
        {
            let xRange = SKRange(constantValue: target.weaponTargetOffset.x)
            let yRange = SKRange(constantValue: target.weaponTargetOffset.y)
        ...
        case is WeaponFiringState:
            ...
            if debugDrawingEnabled
            {
                guard let sourceOrientation = source.component(ofType: OrientationComponent.self) else {
                    fatalError("WeaponNodees must be associated with entities that have an orientation node")
                }
    ...
    func positionLineNode(from source: TaskBot, to target: TaskBot)
    {
        ...
        let sourcePosition: CGPoint = {
            ...
            position.x += source.weaponTargetOffset.x
            position.y += source.weaponTargetOffset.y
        ...
        let destinationPosition: CGPoint = {
            ...
            var position = convert(node.position, from: nodeParent)
            position.x += target.weaponTargetOffset.x
            position.y += target.weaponTargetOffset.y
